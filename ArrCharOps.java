...
public class ArrCharOps {
    ...
    */
    public static char[] concat(char[] arr1, char[] arr2) {
        char[] concatanationArray = new char[arr1.length + arr2.length];
        for (int i = 0; i < arr1.length; i++) {
            concatanationArray[i] = arr1[i];
        }
        for (int i = arr1.length; i < concatanationArray.length; i++) {
            concatanationArray[i] = arr2[i - arr1.length];
        }
        return concatanationArray;
    }
    ...
    public static char[] subArray(char[] arr, int beginIndex, int endIndex) {
        char[] concatanationArray = new char[endIndex - beginIndex];
        if (arr.length == 0 || beginIndex < 0) {
            return null;
        }
        int index = 0;
        for (int i = beginIndex; i < endIndex; i++) {
            concatanationArray[index] = arr[i];
            index++;
        }
        return concatanationArray;
    }
    ...
    public static long hashCode(char[] arr) {
        ...
        long returnedHash = 0;
        for (int i = 0; i < arr.length; i++) {
            returnedHash += (arr[i] * Math.pow(7, arr.length - i -1));
        }
        return returnedHash;
    }
